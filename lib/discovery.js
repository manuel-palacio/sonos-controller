...
function startDiscovery(onDevice) {
  ...
  const query = () => m.query({ questions: [{ name: '_sonos._tcp.local', type: 'PTR' }] });
  query();
  const interval = setInterval(query, 15000);
  return () => { clearInterval(interval); m.destroy(); };
}
