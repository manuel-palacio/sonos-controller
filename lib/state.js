...
class StateStore extends EventEmitter {
  ...
  getDeviceByRincon(id) { return this._devices.get(id); }
    ...
    getRooms() {
        const all = Array.from(this._state.values());
        return all.filter((r) => {
            if (r.id === r.groupCoordinatorId) return true;          // standalone or group coordinator
            const coord = this._state.get(r.groupCoordinatorId);
            return !coord || coord.name !== r.name;                  // different name → real grouped room, keep it
        });
    }
